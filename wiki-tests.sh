...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q http://wiki.apertium.org/wiki/English_and_French/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch http://wiki.apertium.org/wiki/English_and_French/$TESTTYPE"; fi
fi
echo "..."
